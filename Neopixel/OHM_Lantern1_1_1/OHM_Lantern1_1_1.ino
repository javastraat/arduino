//  Copyright (C) 2013 Andy Krenz and Ralph Reijs
//  OHM LANTERN IOS APP by AppCodex
//  
//
//
...
int inputPin = 2;               // choose the input pin for the pushbutton
...
void setup()
{
  ...
  Serial.println("OHM Lantarn 1.1.1"); // so I can keep track of what is loaded
  Serial.print("IP: ");
...
void loop()
{
  ...
  if (client) {
    ...
    Serial.println("Client connected"); // so I can keep track of what is loaded 
    Serial.println("................."); // so I can keep track of what is loaded
    ...
    while (client.connected()) {
      if (client.available()) {
        ...
        if (c == '\n' && currentLineIsBlank) {
          ...
          client.println("<HTML>");
          client.println("<HEAD>");
          client.println("<TITLE>OHM Lantern Controlpage</TITLE>");
          client.println("</HEAD>");
          client.println("<BODY>");
          client.println("<H1>OHM Lantern Controlpage</H1>");
          client.println("<a href=\"/?colorcode/OFF\" target=\"inlineframe\">OFF</a><br>"); 
          client.println("<a href=\"/?colorcode/white\" target=\"inlineframe\">ON (white)</a><br>");
        ...
        if (c == '\n') {
          ...
          //
          if(buffer.indexOf("GET /?mode/rainbow")>=0) 
            rainbowCycle(20); 

          if(buffer.indexOf("GET /?mode/knightrider")>=0) 
            kit(strip.Color(red), 100);

          if(buffer.indexOf("GET /?mode/polizei")>=0) 
            polizei(40);

          if(buffer.indexOf("GET /?mode/fireplace")>=0) 
            rainbowCycle(2); 

          if(buffer.indexOf("GET /?mode/text")>=0) 
            kit(strip.Color(white), 100);

          if(buffer.indexOf("GET /?mode/mail")>=0) 
            kit(strip.Color(green), 100);

          if(buffer.indexOf("GET /?mode/pacman")>=0)
            kit(strip.Color(yellow), 100);

          if(buffer.indexOf("GET /?mode/invaders")>=0)
            kit(strip.Color(green), 100);
          ...
          if(buffer.indexOf("GET /?colorcode/OFF")>=0)
            colorSet(strip.Color(black), 200); 
          if(buffer.indexOf("GET /?colorcode/black")>=0)
            colorSet(strip.Color(black),200);
          if(buffer.indexOf("GET /?colorcode/blue")>=0)
            colorSet(strip.Color(blue),0);
          if(buffer.indexOf("GET /?colorcode/cyan")>=0)
            colorSet(strip.Color(cyan),0);
          if(buffer.indexOf("GET /?colorcode/gray")>=0)
            colorSet(strip.Color(gray),0);
          if(buffer.indexOf("GET /?colorcode/green")>=0)
            colorSet(strip.Color(green),0);
          if(buffer.indexOf("GET /?colorcode/lime")>=0)
            colorSet(strip.Color(lime),	0);
          if(buffer.indexOf("GET /?colorcode/magenta")>=0)
            colorSet(strip.Color(magenta),0);
          if(buffer.indexOf("GET /?colorcode/maroon")>=0)
            colorSet(strip.Color(maroon),0);
          if(buffer.indexOf("GET /?colorcode/navy")>=0)
            colorSet(strip.Color(navy),0);
          if(buffer.indexOf("GET /?colorcode/olive")>=0)
            colorSet(strip.Color(olive),0);
          if(buffer.indexOf("GET /?colorcode/purple")>=0)
            colorSet(strip.Color(purple),0);
          if(buffer.indexOf("GET /?colorcode/red")>=0)
            colorSet(strip.Color(red),0);
          if(buffer.indexOf("GET /?colorcode/silver")>=0)
            colorSet(strip.Color(silver),0);
          if(buffer.indexOf("GET /?colorcode/teal")>=0)
            colorSet(strip.Color(teal),0);
          if(buffer.indexOf("GET /?colorcode/white")>=0)
            colorSet(strip.Color(white),0);
          if(buffer.indexOf("GET /?colorcode/yellow")>=0)
            colorSet(strip.Color(yellow),0);
          if(buffer.indexOf("GET /?colorcode/oranje")>=0)
    ...
    client.stop();  
    Serial.println("End of connection"); // so I can keep track of what is loaded 
    Serial.println("................."); // so I can keep track of what is loaded
...
void buttonPush(uint8_t wait) {
  for(uint16_t i=0; i<strip.numPixels(); i++) {
    ...
    if (val == LOW) {            // check if the input is HIGH
      ...
      Serial.println("Button pressed"); // so I can keep track of what is loaded 
      rainbowCycle(10);
      Serial.println("................."); // so I can keep track of what is loaded
    }
