...
byte ip[] = { 10, 0  , 20, 177 }; // ip in lan
byte gateway[] = { 10, 0, 20, 1 }; // internet access via router
...
void setup()
{
  ...
  Serial.begin(9600); 
  Serial.println("Neopixel ready "); // so I can keep track of what is loaded
  ...
  if (client) {
    ...
    while (client.connected()) {
      if (client.available()) {
         ...
         int colorRequests = 0;  // counters to show the number of pin change requests
         int modeRequests = 0;  // counters to show the number of pin change requests
        ...
        if( finder.find("GET /") ) {            
          ...
          while(finder.findUntil("?color", "\n\r")){  
            char type = client.read(); // =
            Serial.println(type);
            //int mode = finder.getValue();
           long value= finder.getValue();
            //char type = finder.long.getValue(); 
            Serial.print(value);
            //int val = finder.getString(char);
           //if( type == '=') {                 
        ...
        client.print(colorRequests);
        client.print(" digital pin(s) written");
        client.println("<br />");
        client.print(modeRequests);
